  ...
  try {
    ...
    imageId.forEach((element) => {
      ...
      if (foundImageIndex !== -1) {
        ...
        if (
          fs.existsSync(
            path.join(__dirname, `../../public/gallery/${foundImage?.imageUrl}`)
          )
        ) {
          fs.unlinkSync(
            path.join(__dirname, `../../public/gallery/${foundImage?.imageUrl}`)
    ...
    res.status(500).json({ error: 'Eroare la ștergerea imaginii', msg: err })
  }
}
...
export const updateGallery = async (req: IRequest, res: Response) => {
  try {
    ...
    const reorderedImages = order
      ...
      .filter((img): img is typeof gallery.imageArray[number] => Boolean(img))
    ...
    console.error(err)
    res.status(500).json({ error: 'Eroare la actualizarea galeriei', msg: err })
  }
}
